...
| Localisation | Done, English shipped | Codeware `ModLocalizationPackage`; 72 keys in `translations/English.reds`. No TweakXL, no locale JSON. Read via the global `GetLocalizedText`. Another language = one file + one line in `Provider.reds` |
