...
| Search expressions | Done | **World Builder's grammar term for term** (`miscUtils.matchSearch`). Flat, no precedence, no brackets: each operator governs the word AFTER it, the first word is a `\|` word, and the answer is (some `\|` word hit) AND (every `&` word hit) AND (no `!` word hit). Whole query tried as a plain substring first, so a phrase still works. An empty OR pool counts as satisfied, so `!corpo` alone means everything except corpo - the one departure, and it can only affect queries that open with an operator, which World Builder answers with an empty list. Spaces belong to the word, as upstream |
| Search syntax panel | Done | Hover-only `[ i ]` beside the input; built once, hidden, parented after both columns so it draws over the cards. Sizes itself - `SetFitToContent` is on `inkWidget`, so the canvas fits and no line carries a wrap. No click handler: a click would cost the text input its focus |
...
| Area names translated | Done | A registry district/subdistrict name is English DATA, not a lockey, so it is translated by the core rather than by a slot file: `NCZDG_LocalizeArea` over `LocalizeArea`, which reads the game's own `District_Record`. Covers the nav, the status line's `{area}`, the district-enter banner and the fast-travel panel. **Raises the minimum core to 1.2.0.** The world map panel needs nothing - it draws counts under the game's own district name |
...
| Installed-mod filter | Done, verified in-game | ALL / INSTALLED / MISSING / **UNKNOWN**; needs NCZoningCore 1.1.0, which detects in redscript over RedFunctions - no CET. Button **hidden** until the scan has answered. Filters the district column and its counts too. **UNKNOWN** covers AMM mods and mods shipping only ArchiveXL .xl files, neither of which any archive query can see |
